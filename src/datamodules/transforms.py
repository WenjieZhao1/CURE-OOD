...
class ExtractPatch:
    ...
    def __init__(self, patch_size, p_tumor=0.5):
        ...
        self.ZeroPadding= ZeroPadding(patch_size)

    def __call__(self, sample):
        img = sample['input']
        mask = sample['target_mask']

        if img.shape[:-1] == self.patch_size:
            return img
        ...
        assert all(x == y for x, y in zip(self.patch_size, patch_img.shape[:-1])), \
...
class AddNoise3D:
    """Adds configurable noise to test-time volumes using CIFAR-C style schedules."""

    _GAUSSIAN_SIGMAS = [0.04, 0.06, 0.08, 0.09, 0.10]
    _SHOT_LEVELS = [500, 250, 100, 75, 50]
    _IMPULSE_AMOUNTS = [0.01, 0.02, 0.03, 0.05, 0.07]
    _SPECKLE_SCALES = [0.06, 0.10, 0.12, 0.16, 0.20]

    def __init__(self, noise_type: str = "gaussian", severity: int = 1, probability: float = 1.0):
        self.noise_type = noise_type.lower()
        supported = {"gaussian", "shot", "impulse", "speckle"}
        if self.noise_type not in supported:
            raise ValueError(f"Unsupported noise_type '{noise_type}'. Supported values: {sorted(supported)}")
        self.severity = int(max(1, min(5, severity)))
        self.probability = float(min(max(probability, 0.0), 1.0))
        self._noise_map = {
            "gaussian": self._apply_gaussian,
            "shot": self._apply_shot,
            "impulse": self._apply_impulse,
            "speckle": self._apply_speckle,
        }
    ...
    def __call__(self, sample):
        ...
        normalized = (image_np - min_val) / max(scale, 1e-8)
        noisy = self._noise_map[self.noise_type](normalized)
        noisy = np.clip(noisy, 0.0, 1.0)

        restored = noisy * scale + min_val
        target_dtype = getattr(image, "dtype", image_np.dtype)
        sample["input"] = restored.astype(target_dtype, copy=False)

        return sample

    def _apply_gaussian(self, array: np.ndarray) -> np.ndarray:
        sigma = self._GAUSSIAN_SIGMAS[self.severity - 1]
        return array + np.random.normal(loc=0.0, scale=sigma, size=array.shape)

    def _apply_shot(self, array: np.ndarray) -> np.ndarray:
        level = self._SHOT_LEVELS[self.severity - 1]
        return np.random.poisson(array * level) / level

    def _apply_impulse(self, array: np.ndarray) -> np.ndarray:
        amount = self._IMPULSE_AMOUNTS[self.severity - 1]
        return skimage_util.random_noise(array, mode="s&p", amount=amount)

    def _apply_speckle(self, array: np.ndarray) -> np.ndarray:
        scale = self._SPECKLE_SCALES[self.severity - 1]
        return array + array * np.random.normal(loc=0.0, scale=scale, size=array.shape)
