...
class Swin(nn.Module):
    def __init__(self, hparams, embed_dim=96, patch_size=4, in_chans=2, img_size=(48,80,80)):
        '''
        TransMorph Model
        '''
    ...
    def forward(self, sample):
        ...
        out_feats = self.transformer(img)[-1]

        out_feats = self.avg_pool(out_feats)

        out_feats = self.flat(out_feats)

        ftr_concat = torch.cat((out_feats, clin_var), dim=1)
