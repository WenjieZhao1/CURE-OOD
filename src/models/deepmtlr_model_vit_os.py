...
class DEEP_MTLR(LightningModule):
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        if hasattr(self.model, "get_mtlr_features") and all(
            ...
            loss = mtlr_neg_log_likelihood(preds1, y1.float(), self.model, self.hparams['C1'], average=True)
        else:
            loss, preds1, preds2, preds3, preds4, y1, y2, y3, y4, labels = self.step(batch)
            features = None
    ...
    def test_epoch_end(self, outputs: List[Any]):
        ...
        # Optional raw-logit export for downstream OOD scoring.
        if os.environ.get("SAVE_OS_LOGITS", "0") == "1":
            np.save("Test_MTLR_Logits_OS.npy", pred_prob1.detach().cpu().numpy())
            np.save("Test_Separate_Logits_OS.npy", separate_logits1.detach().cpu().numpy())
