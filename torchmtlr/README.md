...
This package provides an `MTLR` class, an implementation of the log likelihood function for training and some handy utility functions. This bundled project variant returns both cumulative MTLR logits and separate interval logits from `MTLR.forward()` so the latter can be used for OOD scoring.
...
# forward pass
logits, separate_logits = model(x)
