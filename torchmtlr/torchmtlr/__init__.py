...
class MTLR(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        tuple of torch.Tensor
            The cumulative MTLR logits with shape
            ``(num_samples, num_time_bins)`` and the separate interval logits
            with shape ``(num_samples, num_time_bins - 1)``.
        """
        out = torch.matmul(x, self.mtlr_weight) + self.mtlr_bias
        return torch.matmul(out, self.G), out
